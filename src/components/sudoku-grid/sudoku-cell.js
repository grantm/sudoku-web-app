...
function hasPencilledDigit(cell, d) {
  return (
    cell.get("innerPencils").includes(d) || cell.get("outerPencils").includes(d)
  );
}
...
function SudokuCell({
  ...
  if (matchDigit !== "0") {
    if (
      digit === matchDigit ||
      (showPencilmarks && hasPencilledDigit(cell, matchDigit))
    ) {
      classes.push("matched");
    }
  }
  if (cell.get("row") === selectedRow) {
    classes.push("highlight");
  }
  if (cell.get("col") === selectedCol) {
    classes.push("highlight");
  }
  const outerPencilmarks = showPencilmarks ? (
    <CellOuterPencilMarks cell={cell} />
  ) : null;
  const innerPencilmarks = showPencilmarks ? (
